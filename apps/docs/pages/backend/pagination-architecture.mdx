...
```mermaid
graph TB
    Client[Client Application] -->|HTTP Request| Controller[Controller]
    Controller -->|PaginationQueryDto| Service[Service Layer]
    Service -->|parsePaginationQuery| PaginationService[PaginationService]
    PaginationService -->|PaginationOptions| Service
    Service -->|buildPrismaOptions| PaginationService
    PaginationService -->|Prisma Options| Service
    Service -->|findMany + count| Prisma[Prisma Client]
    Prisma -->|Query| Database[(Database)]
    Database -->|Results| Prisma
    Prisma -->|data + total| Service
    Service -->|createPaginatedResponse| Response[Paginated Response]
