...
The pagination utility provides a robust, reusable solution for implementing data pagination, filtering, sorting, and field selection across all NestJS services.
...
async findAll(@Query() query?: PaginationQueryDto) {
  ...
  const [data, total] = await Promise.all([
    this.prisma.character.findMany(prismaOptions),
...
- [Quick Reference](/backend/pagination-quick-ref) - Fast reference card
- [Architecture](/backend/pagination-architecture) - System design
